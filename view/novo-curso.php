<?php
    ...
    if(isset($_POST['nameCourse']) && !empty($_POST['nameCourse']) && empty($_GET['id'])) {
        ...
        if($result) {
            header("Location: ?p=cursos&mensagem=Dados salvos com sucesso&tipo=success");
        } else {
            header("Location: ?p=cursos&mensagem=Ocorreu um erro&tipo=error");
        }
    }
    ...
    if(isset($_POST['nameCourse']) && isset($_GET['id'])) {
        ...
        if($result) {
            header("Location: ?p=cursos&mensagem=Dados alterados com sucesso&tipo=success");
        } else {
            header("Location: ?p=cursos&mensagem=Ocorreu um erro&tipo=error");
        }
    }
